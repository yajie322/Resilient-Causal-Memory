...
func userInput(write_chan chan bool, read_chan chan string) {
	...
	for {
		...
		if strings.HasPrefix(text, "write") {
			...
			status = false
			msg := Message{Type: QUIT, Id: -1, Key: 0, Val: "", Vec: make([]int, 1)}
			broadcast(&msg)
			break
		}
	}
...
func write(key int, value string, done chan bool) {
	...
	msg := Message{Type: CLIENT_WRITE, Id: -1, Key: key, Val: value, Vec: make([]int, 1)}
	// chose server
...
func read(key int, done chan string) string {
	// create message object
	msg := Message{Type: CLIENT_READ, Id: -1, Key: key, Val: "", Vec: make([]int, 1)}
	// chose server
	chosen := rand.Intn(len(mem_list))
	// send
	send(&msg, mem_list[chosen])
